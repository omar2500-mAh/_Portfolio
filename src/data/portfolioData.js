...
const Hero = () => (
  <section className="bg-gray-100 py-24 text-center">
    ...
    />
    <h1 className="text-4xl font-bold mb-4 whitespace-pre-line">{heroData.title}</h1>
      ...
      {heroData.ctaButtons.map((btn, i) => (
        <a
          ...
          className={`px-6 py-2 rounded-md font-medium ${
            btn.primary ? "bg-green-600 text-white hover:bg-green-700" : "bg-gray-200 text-gray-900 hover:bg-gray-300"
          }`}
        >
          {btn.label}
        </a>
    ...
    <div className="max-w-7xl mx-auto px-4">
      <div className="text-center mb-12">
        ...
        <p className="mt-4 text-lg text-gray-600 max-w-2xl mx-auto">
          Combining research, engineering practice, and applied projects to advance battery technologies, BMS systems, EV energy storage, and innovative embedded solutions.
        </p>
      </div>
            ...
            <div className="p-6">
              ...
              {item.tags && (
                <div className="flex flex-wrap gap-2 mb-4">
                  {item.tags.map((tag, i) => (
                    <span key={i} className="px-3 py-1 bg-green-100 text-green-800 text-xs rounded-full">{tag}</span>
                  ))}
                </div>
              )}
              {item.buttonLink && item.buttonText && (
                <a
                  ...
                  className="inline-block mt-2 px-5 py-2 bg-green-600 text-white rounded-lg font-medium hover:bg-green-700 transition"
                >
                  {item.buttonText}
                </a>
              )}
...
const Awards = () => (
  <section className="py-16 bg-white" id="awards">
    <div className="max-w-7xl mx-auto px-4 text-center">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
        {awardsCertificates.map((item, i) => (
          <div key={i} className="bg-gray-50 rounded-xl shadow-md p-6 hover:shadow-xl transition">
  ...
  <section className="py-16 bg-gray-50" id="experience">
    <div className="max-w-5xl mx-auto px-4 text-center">
      ...
      {experience.map((exp, i) => (
        <div key={i} className="mb-6 bg-white rounded-xl shadow-md p-6 hover:shadow-xl transition">
    ...
    <div className="max-w-5xl mx-auto px-4 text-center">
      ...
      <div className="flex justify-center gap-4 mt-4">
        {socialLinks.map((link, i) => (
          <a key={i} href={link.url} className="text-gray-600 hover:text-gray-900">{link.platform}</a>
        ))}
      </div>
    </div>
  </section>
